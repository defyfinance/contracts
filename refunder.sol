...
contract DefyRefunder is Ownable, ReentrancyGuard {
    ...
    // Distribution status
    bool public distributionEnd;
    // distribution start
    uint256 public startTime;
    // end timestamp
    uint256 public endTime = 1654041599 ; // 31 May 2022 11:59:59 PM GMT
    ...
    event DistributeUSDC();
    event Claim(address User, uint256 amount);
    event Distribution(bool);
    ...
    function endDistribution(bool status) onlyOwner external {
        require(block.timestamp > endTime , "cannot stop the distribution before endTime");
        distributionEnd = status;
        emit Distribution(status);
    }
    ...
    function addUsers(addUser[] calldata holders) external onlyDev returns(bool){
        require(block.timestamp < startTime , "distribution already started!");
        ...
        for (uint i = 0; i < length; i++) {
			...
			require(!userList[_user].inList , "user already added" );
            require(!distributionEnd, "distibution has been ended");

            userAddrList.push(_user);
            userList[_user].amount = _amount;
            userList[_user].index = userAddrList.length - 1 ;
            userList[_user].inList = true;
    ...
    function distributeUSDC() external onlyDev returns(bool){
        ...
        for (uint i = 0 ; i < userAddrList.length ; i++)
        {
            ...
            uint256 amount = userList[addr].amount;
            if (amount > 0){
                safeDefyTransfer(addr, amount); 
                totalUSDC = totalUSDC.sub(amount);
                userList[addr].amount = 0;
            }
    ...
    function claimRefund() external returns(bool){
        ...
        totalUSDC = totalUSDC.sub(pending);
        userList[msg.sender].amount = 0; 
    ...
    function safeDefyTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > usdcBal) {
            successfulTansfer = usdc.transfer(_to, usdcBal);
        } else {
            successfulTansfer = usdc.transfer(_to, _amount);
        }
        require(successfulTansfer, "safeDefyTransfer: transfer failed");
    }
    
    function withdrawRemainder(address receiver)external onlyOwner {
        require(distributionEnd, "only withdrawable after distribution end"); 
        safeDefyTransfer(receiver, usdc.balanceOf(address(this)));
    }
