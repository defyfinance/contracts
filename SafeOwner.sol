...
interface DefyMaster {
    ...
    function poolInfo(uint256 pid) external view returns (address lpToken, address stubToken, uint256 allocPoint, uint256 allocPointDR,
        uint256 depositFee, uint256 withdrawalFee, uint256 lastRewardTimestamp, uint256 lastRewardTimestampDR,
        uint256 rewardEndTimestamp, uint256 accDefyPerShare, uint256 accSecondRPerShare, uint256 lpSupply,
        bool impermanentLossProtection, bool issueStub);
...
contract SafeDefyOwner is Ownable {
    ...
    try IDefySwapPair(_lpToken).token1() returns (address token1) {
        ...
        require(_lpToken != secondR , "Add: can't use secondR as a lp token");

        defyMaster.add(_allocPoint, _allocPointDR, IERC20(_lpToken), _stub, _token0, _token1, _depositFee, _withdrawalFee,
    ...
    (address lpToken, , , , , , , , , , , , , ) = defyMaster.poolInfo(_pid);
